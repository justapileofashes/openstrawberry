...
LINUX_OUTPUT = ROOT / "resources" / "linux-icons"
...
def main() -> None:
    ...
    with Image.open(SOURCE) as source:
        ...
        for size in (16, 32, 48, 64, 128, 256, 512):
            image.resize((size, size), Image.Resampling.LANCZOS).save(LINUX_OUTPUT / f"{size}x{size}.png", format="PNG", optimize=True)
        image.save(ICO, format="ICO", sizes=[(size, size) for size in sizes])
    mac_sources = [str(OUTPUT / name) for _, name in mac_representations]
    subprocess.run(["icnsutil", "compose", "--force", "--toc", str(ICNS), *mac_sources], check=True)
