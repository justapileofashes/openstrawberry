...
export class ProviderRunner {
  ...
  public async run(request: AgentRunRequest): Promise<AgentRunResult> {
    ...
    try {
      ...
      if (!baseUrl.startsWith("https://")) throw new Error("Provider endpoints must use HTTPS.");
      ...
      return { agentId: request.agentId, provider: "redacted", model: "redacted", text: "", startedAt, completedAt: Date.now(), status: "failed", error: error instanceof Error ? error.message : "Provider run failed." };
    }
  }
}
