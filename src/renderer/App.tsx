  ...
  return <div className="app-shell">
    ...
    <main className="browser-shell">
      <header className="product-bar">
        ...
        <span className="icon-control-wrap"><button className={`update-trigger ${updateState.status}`} aria-label={updateState.status === "downloaded" ? "Restart to install browser update" : updateState.status === "available" ? "Browser update available" : "Browser updates"} onClick={() => setUpdatePanelOpen(true)}><RefreshCw size={15} /></button><span className="control-tooltip" role="tooltip">{updateState.status === "downloaded" ? "Restart to install update" : updateState.status === "available" ? "Update available" : "Browser updates"}</span></span>
        <span className="icon-control-wrap"><button className="agents-trigger" aria-label="Open Agent workspace" onClick={() => setAgentRailOpen((value) => !value)}><Bot size={15} /></button><span className="control-tooltip" role="tooltip">Agent workspace</span></span>
      </header>
